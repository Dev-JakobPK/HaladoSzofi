...
namespace Beadando_JakobPKristof_KatonaBence
{
    internal class Program
    {
        ...
        {
            string json = JsonConvert.SerializeObject(jwst, Newtonsoft.Json.Formatting.Indented);
        ...
        static void dbMentes(string dbPath, string datum, double homereseklet1, double homereseklet2, double sugarzas)
        {
            using (SqliteConnection connection = new SqliteConnection($"Data Source={dbPath}"))
            {
                connection.Open();
        ...
        static void AdatokMentese(string dbPath)
        {
            if (dbPath == null || string.IsNullOrEmpty(dbPath.ToString()))
            {
                Console.WriteLine("Hiba: Az adatbázis elérési útja nem lett megadva.");
            ...
            foreach (var item in jwst)
            {
                ...
                dbMentes(dbPath, datum, ho1, ho2, sugarzas);
            }
        ...
        static void Main()
        {
            ...
            Batteries.Init();
            string dbPath = "C:\\Users\\Tanuló\\source\\repos\\HaladoSzofi\\adatbazis.db";
            AdatbazisLetrehozasa(dbPath);
            AdatokMentese(dbPath);
            Console.ReadKey();
        }
    }
}
